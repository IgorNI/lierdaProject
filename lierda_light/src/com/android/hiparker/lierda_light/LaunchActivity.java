		...
		if (enable) {
			mHandler.postDelayed(new Runnable(){
				...
				public void run() {
					...
					finish();
					String version = AppUtil.getVersionName(getApplicationContext());
					if (version.equals(SpUtil.getAppVersion(getApplicationContext()))) {
						Intent intent = new Intent(LaunchActivity.this, HomeActivity.class);
						startActivity(intent);
					} else {
						Intent intent = new Intent(LaunchActivity.this, FeatureActivity.class);
						startActivity(intent);
					}
				}
			}, SCAN_PERIOD);
