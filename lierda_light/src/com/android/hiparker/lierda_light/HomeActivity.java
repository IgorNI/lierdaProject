...
//		sm.setTouchModeAbove(SlidingMenu.TOUCHMODE_NONE);
		...
		enableRightBtn("管理", new View.OnClickListener() {
			@Override
...
//	    	}
			if (mLight.getStatus() != Light.STATE_CONNECTED) {
				...
				if (mLight.getStatus() == Light.STATE_DISCONNECTED) {
					if (mLight.mBluetoothGatt == null) {
						mLight.connect(HomeActivity.this, mHandler);
					} else {
...
//	    	}
	...
	private void initGroup() {
		...
		for (Light light : LightManager.getInstance().getGroupLights()) {
	...
	private void initLightParam() {
		mLight.readCharacteristic(new Light.OnValueReaded() {
			...
			public void onReaded(byte[] values) {
				final int openStatus = values[0]&0xFF;
				final int lightness = (values[1]&0xFF) * 100 /0xFF;
				final int colorTemp = (values[2]&0xFF) * 100/0xFF;
				...
				runOnUiThread(new Runnable() {
					...
					public void run() {
						...
						mControlView.setSwitchStatus(openStatus>0?1:0);
					}
				});
	...
	private void initActionBar() {
		...
		//进度
		mPbProgress = findViewById(R.id.base_id_progress);
		mPbProgress.setOnTouchListener(new View.OnTouchListener() {
			@Override
